    ...
    with gr.Tab("🧹 Manage & help"):
        ...
        with gr.Accordion("⚙️ Settings (hardware & optimization)", open=False):
            gr.Markdown(
                ...
                "a question you are asked: sd.cpp\nplans where each "
                "module's weights live — card, then RAM, then another card, "
                "then\ndisk — and the **benchmark button measures it "
                "against the others on your\nmachine**, on a single card "
                "too, instead of asking you to bet. Note that a card without tensor cores (Pascal, "
        ...
        with gr.Accordion("🧊 Text / Image → 3D (trellis)", open=False):
            gr.Markdown(
                ...
                "on its command line: auto is reached by sending *nothing*. "
                "It keeps\nan image's existing alpha — the Toolkit's cutout, "
                "for instance — and runs BiRefNet\nonly when there is none. "
                "The **threshold** option is kept but labelled: it reads "
                "bright\nspecular highlights as background, and the model "
                "then generates **holes** right there.\n\n"
                "**WebP textures** (Advanced) — on by default, via the "
                "`EXT_texture_webp` glTF\nextension, which not every viewer "
                "or marketplace reads. Untick for PNG and a GLB\nanything "
                "opens.\n\n**Decimation** — a legacy cluster GRID, not a "
                "face count. Left at 0 you get the\ncurrent default instead: "
                "quadric simplify to 150K faces at 512 (300K at "
        ...
        with gr.Accordion("🔄 Update the app",
                          open=False):
            gr.Markdown(
                "**There is one update button, and it does four things**, in "
                "this order:\n\n"
                "1. the **code**, from GitHub;\n"
                "2. the **cleanup** — what removed features left behind, then "
                "a check that\n   everything compiles and the catalog holds "
                "together;\n"
                "3. the **sd.cpp engine** (images and video), brought in line "
                "with that code;\n"
                "4. the **3D engine** (trellis.cpp) — the binary only, and "
                "only if it is\n   already installed. The ~16 GB of 3D "
                "weights are never re-downloaded.\n\n"
                "The order is the point. Everything after step 1 has to run "
                "on the NEW code:\nit is the code that says which options "
                "`sd-cli` must know. There used to be four\nbuttons for "
                "this, in an order nothing announced, and three of them were "
                "always\nforgotten — so: an engine one release behind, a "
                "folder that only grows, and\nfailures whose cause was an "
                "update half-done weeks earlier.\n\n"
                "**The cleanup measures first**: it shows the total it could "
                "free and **asks**\nbefore deleting anything. Answer no and "
                "nothing is deleted.\n\nEach step is still reachable on its "
                "own when you really want one without the\nothers: "
                "`update.bat --clean`, `--engine`, `--trellis`, `--code`.\n\n"
                "**The code step** downloads the current code\nof **the "
                "branch this install came from** — printed before anything is "
                "fetched, and changed\nonly with `update.bat --branch "
                "<name>` — and applies it in place: it writes only the files "
                "that actually\ndiffer, deletes what disappeared from the "
                "project (and only files it installed\nitself), backs up "
                "everything it replaces (`update.bat --rollback` undoes "
                "the\nupdate), and restores the previous version by itself "
                "if the new code does not\ncompile. Close the app first — "
                "Windows cannot replace a file that is open.\n\n"
                "**What is never touched:** `models/`, `loras/`, `outputs/`, "
                "`userdata/`,\n`tools_repo/`, `bin/`, `python/`. Your "
                "models, images, settings and engines are\nout of its reach "
                "by construction.\n\n⚠️ **One trap worth knowing.** A "
                "Toolkit add-on is not just a folder of\nweights: its "
                "installer also pins Python package versions shared with the "
                "other\nadd-ons. When an update changes *how an add-on "
                "installs*, updating the app\nleaves that add-on frozen in "
                "its old state — and it only shows at the next use.\nIn that "
                "case, **click its “Install” button again**; "
                "already-downloaded weights\nare not fetched twice. The "
                "cleanup names precisely what is stale, so you do not\nhave "
                "to guess.")
        ...
        with gr.Accordion("🌐 Updating & maintenance", open=False):
            gr.Markdown(
                ...
                "the only update button. Code, then cleanup, then the "
                "sd.cpp\nengine, then the 3D engine if it is installed. The "
                "~16 GB of 3D weights are\nnever re-downloaded. It replaces "
                "`maintenance.bat`, `update-engine.bat` and\n"
                "`update-trellis.bat`, which are gone — and which it deletes "
                "from your folder\nthe first time it runs, so no one "
                "double-clicks a button that does a quarter of\nthe work "
                "without saying so.\n**📁 Model location** "
