    ...
    with gr.Tab("🧹 Gestion & aide"):
        ...
        with gr.Accordion("🎨 Onglets de génération (Flux.2 / Krea 2 / Boogu)",
            ...
            gr.Markdown(
                ...
                "idée en prompt anglais (add-on à installer). Le sélecteur "
                "**Style** choisit entre *Détaillé* (paragraphe descriptif "
                "complet) et ***Midjourney*** (court et dense, phrases "
                "juxtaposées, l'esthétique d'abord). **Propositions** en génère "
                "plusieurs d'un coup : clique celle que tu préfères. "
                "**Stylize** = licence artistique (style Midjourney seulement) ; "
                "**Chaos** = écart entre les propositions.  \n"
                "Tu peux aussi taper les **paramètres Midjourney** dans le "
                "prompt — `--ar 16:9` (format à surface constante), "
                "`--stylize 500`, `--chaos 40`, `--no voitures` — ils sont "
                "retirés du texte et appliqués aux réglages, même sans passer "
                "par l'améliorateur. Réglages **et** installation de l'add-on "
                "sont dans le même repli « ✨ Améliorateur de prompt ».\n\n"
        ...
        with gr.Accordion("🖼️ Outpaint (étendre une image)", open=False):
            gr.Markdown(
                ...
                "**Comment ça marche** — la toile est agrandie, les nouvelles "
                "zones sont pré-remplies à partir des pixels du contour, un "
                "**masque d'inpainting** est fourni au moteur *s'il sait le "
                "gérer* (l'option est détectée automatiquement sur ton binaire), "
                "le tout passe en **image-to-image**, la **tonalité du neuf est "
                "recalée** sur celle de l'original, puis **l'image d'origine est "
                "recollée** par-dessus.\n\n"
                ...
                "**Remplissage des bords** — *Étirement flou* (défaut) prolonge "
                "les pixels du contour : ça ne transmet que la couleur, aucune "
                "forme. *Miroir* donne une continuité parfaite sur un motif "
                "régulier, **mais reflète un sujet proche du bord** — et le "
                "modèle transforme volontiers ce reflet en un second objet bien "
                "réel. À réserver aux fonds uniformes.  \n"
                "**Force de génération** — combien le modèle réinvente la "
                "nouvelle zone. Haut (0.8–1.0) = invente librement ; bas = reste "
                "proche du pré-remplissage (utile pour du ciel, du sable, une "
                "texture uniforme).  \n"
